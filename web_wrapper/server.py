...
def create_app(client: Twitch) -> FastAPI:
    ...
    def _require_game(payload: dict[str, Any]) -> str:
        value = payload.get("game")
        if not isinstance(value, str):
            raise HTTPException(status_code=400, detail="Field 'game' must be a non-empty string")
        name = value.strip()
        if not name:
            raise HTTPException(status_code=400, detail="Field 'game' must be a non-empty string")
        return name

    def _require_position(payload: dict[str, Any]) -> Optional[int]:
        if "position" not in payload or payload["position"] in (None, ""):
            return None
        try:
            raw_value = int(payload["position"])
        except (TypeError, ValueError) as exc:  # pragma: no cover - defensive branch
            raise HTTPException(status_code=400, detail="position must be a positive integer") from exc
        if raw_value < 1:
    ...
    @app.post("/settings/priority/add")
    async def add_priority(payload: dict[str, Any]) -> JSONResponse:
        game = _require_game(payload)
        position = _require_position(payload)
        try:
            values, action = client.gui.settings.add_priority(game, position=position)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        position_label: Optional[int] = None
        if game in values:
            position_label = values.index(game) + 1
        if action == "added":
            message = (
                f"{game} added to priority list"
                + (f" at position {position_label}." if position_label else ".")
            )
        elif action == "moved":
            message = (
                f"{game} moved to position {position_label} in priority list"
                if position_label
                else f"{game} moved in priority list."
            )
        else:
            message = f"{game} is already in the priority list."
        return JSONResponse(
            {
                "status": "ok",
                "action": action,
    ...
    @app.post("/settings/priority/remove")
    async def remove_priority(payload: dict[str, Any]) -> JSONResponse:
        game = _require_game(payload)
        try:
            values, action = client.gui.settings.remove_priority(game)
        except ValueError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        message = f"{game} removed from priority list."
        return JSONResponse(
            {
                "status": "ok",
                "action": action,
    ...
    @app.post("/settings/exclude/add")
    async def add_exclusion(payload: dict[str, Any]) -> JSONResponse:
        game = _require_game(payload)
        try:
            values, action = client.gui.settings.add_exclusion(game)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
        if action == "added":
            message = f"{game} added to exclusion list."
        else:
            message = f"{game} is already excluded."
        return JSONResponse(
            {
                "status": "ok",
                "action": action,
    ...
    @app.post("/settings/exclude/remove")
    async def remove_exclusion(payload: dict[str, Any]) -> JSONResponse:
        game = _require_game(payload)
        try:
            values, action = client.gui.settings.remove_exclusion(game)
        except ValueError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        message = f"{game} removed from exclusion list."
        return JSONResponse(
            {
                "status": "ok",
                "action": action,
