...
class SettingsPanel:
    ...
    def add_priority(self, game: str, *, position: Optional[int] = None) -> tuple[list[str], str]:
        ...
        if position is not None:
            if position < 0:
                position = 0
            elif position > len(priority):
                position = len(priority)

        if name in priority:
            current_index = priority.index(name)
            if position is None or position == current_index:
                self.refresh()
            ...
            if position is None:
                ...
                position -= 1
            priority.insert(position, name)
            self._settings.priority = priority
            self._settings.save()
            self.refresh()
            return priority, "moved"

        insert_at = len(priority) if position is None else position
        priority.insert(insert_at, name)
        self._settings.priority = priority
        self._settings.save()
        self.refresh()
        return priority, "added"

    def remove_priority(self, game: str) -> tuple[list[str], str]:
        name = self._normalize_game(game)
        priority = list(getattr(self._settings, "priority", []))
        if name not in priority:
            raise ValueError(f"{name} not found in priority list")
    ...
    def add_exclusion(self, game: str) -> tuple[list[str], str]:
        name = self._normalize_game(game)
        exclude_values = set(getattr(self._settings, "exclude", set()))
        if name in exclude_values:
            self.refresh()
            return sorted(exclude_values), "exists"
        exclude_values.add(name)
        self._settings.exclude = exclude_values
        self._settings.save()
        self.refresh()
        return sorted(exclude_values), "added"

    def remove_exclusion(self, game: str) -> tuple[list[str], str]:
        name = self._normalize_game(game)
        exclude_values = set(getattr(self._settings, "exclude", set()))
        if name not in exclude_values:
            raise ValueError(f"{name} not found in exclusion list")
        exclude_values.remove(name)
        self._settings.exclude = exclude_values
        self._settings.save()
        self.refresh()
